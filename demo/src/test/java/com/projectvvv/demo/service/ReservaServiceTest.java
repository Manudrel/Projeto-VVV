...
class ReservaServiceTest {
    ...
    void deveBuscarReservaPorIdComSucesso() {
        ...
        Mockito.when(reservaRepository.findById(id))
    ...
    void deveBuscarReservasPorStatus() {
        ...
        List<Reserva> listaEsperada =
                List.of(new Reserva(), new Reserva());
    ...
    void deveCriarReservaComSucesso() {
        ...
        Mockito.when(reservaRepository.save(any(Reserva.class)))
                .thenReturn(reservaSalva);

        Reserva resultado = reservaService.criar(dto);
    ...
    void deveAtualizarCamposEnviados() {
        ...
        ReservaDTO dadosNovos = new ReservaDTO();
        ...
        Reserva resultado =
                reservaService.atualizar(id, dadosNovos);

        assertEquals(
                "LOC999",
                resultado.getLocalizador());

        assertEquals(
                StatusReserva.PENDENTE,
                resultado.getStatusReserva());
    }
    ...
    void deveDeletarReservaComSucesso() {
        ...
        Mockito.when(
                        reservaRepository.existsById(id))
                .thenReturn(true);
